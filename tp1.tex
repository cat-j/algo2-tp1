...
\tadNombre{Casillero} es \tadNombre{Nat}
...
\tadOperacion{contarNoDominadosHasta}{jugador/j,nat,partida/p}{nat}{$j \leq \text{\#jugadores($p$)}$}
...
\tadAxioma{fichasEnCasillero($c,$ mover($j,m,n,p$))}{\IF est\'aDominado?($c,p$) THEN {\IF $j \in \text{fichasEnCasillero(c,p)} \land m \in \text{todosLosMovs($c$,tablero($p$))}$ THEN (fichasEnCasillero($c,p$) - agNVeces($j,n,\emptyset$)) $\cup$ \\ fichasVecinasDeJ($c,j,m,p$) ELSE fichasEnCasillero($c,p$) $\cup$ fichasVecinasDeJ($c,j,m,p$) FI} ELSE {\IF est\'aDisputado?(fichasEnCasillero($c,p$)) THEN sinUno(fichasEnCasillero($c,p$)) $\cup$ fichasVecinasDeJ($c,j,m,p$) ELSE fichasEnCasillero($c,p$) $\cup$ fichasVecinasDeJ($c,j,m,p$) FI} FI}
...
\begin{flushright}
\textit{//La funci\'on maxiFourcade te devuelve al m\'as ganador de todos. Si no lo conoc\'es, googlealo.}
\end{flushright}
...
\section{Funciones auxiliares de otros TADs}

\tadOperacion{agNVeces}{$\alpha$,nat,multiconj($\alpha$)}{multiconj($\alpha$)}{}
\tadAxioma{agNVeces($a,n,c$)}{\IF $n=0$ THEN $c$ ELSE Ag($a,$agNVeces($a,n-1,c$)) FI}

\tadOperacion{$\argumento [ \argumento ]$}{secu($\alpha$),nat}{$\alpha$}{$n < $ long($s$)}
\tadAxioma{s[n]}{\IF $n=0$ THEN prim($s$) ELSE fin($s$)[$n-1$] FI}

\tadOperacion{aConj}{multiconj($\alpha$)}{conj($\alpha$)}{}
