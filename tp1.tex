...
\begin{flushright}
\textit{//agregarCasillero: grega un casillero del continente k conectado por el movimiento m al casillero c. El movimiento m' conecta c al casillero creado. Las restricciones en k aseguran que se cumpla el agrupamiento de continentes, las restricciones sobre m y m' aseguran que se cumpla la unicidad de movimientos y la necesidad de dos movimientos asegura que se cumpla la simetr\'ia.}
\end{flushright}
...
\begin{flushright}
\textit{//todosLosMovs: devuelve un conjunto con todos los movimientos que salen del casillero c a cualquier casillero del tablero.}
\end{flushright}
...
\end{flushright}
\tadOperacion{fichasEnCasillero}{casillero/c,partida/p}{multiconjExt(jugador)}{$c \leq \text{\#casilleros(tablero($p$))}$}
...
\end{flushright}
\tadOperacion{crearPartida}{tablero/t,nat/js,secuExt(casillero)/cs,secuExt(continente)/ks}{partida}{$2 \leq js \land \text{long($cs$)} = \text{long($ks$)} = js \land \text{sinRepetidos($cs$)} \land ((\forall k:string) \text{est\'a?($k,ks$)} \Rightarrow $\\$ k \in \text{continentes($t$)}) \land ((\forall c:nat) \text{est\'a?($c,cs$)} \Rightarrow c \leq \text{\#casilleros($t$)})$}

\begin{flushright}
\textit{//agregarFicha: agrega una ficha del jugador j en el casillero c. Requiere que est\'e vac\'io o dominado por j.}
\end{flushright}
\tadOperacion{agregarFicha}{jugador/j,casillero/c,partida/p}{partida}{$j \leq \text{\#jugadores($p$)} \yluego \text{est\'aActivo?($j,p$)} \land \neg \text{terminada?($p$)} \land c \leq \text{\#casilleros(tablero($p$))} \yluego $\\$ \text{jugadoresEnCasillero($c,p$)} = \emptyset \lor \text{jugadoresEnCasillero($c,p$)} = \{j\} $}
\begin{flushright}
...
\begin{flushright}
\textit{//fichasVecinasDeJ: dado un casillero c, devuelve un multiconjunto con todas las fichas de j de todos los casilleros del tablero que con el movimiento m podr\'ian llegar a c.}
\end{flushright}
\tadOperacion{fichasVecinasDeJ}{casillero/c,jugador/j,movimiento/m,partida/p}{multiconjExt(jugador)}{$c \leq \text{\#casilleros(tablero($p$))}$}
...
\tadOperacion{complet\'oLaMisi\'on?}{jugador/j,partida/p}{bool}{$j \leq \text{\#jugadores($p$)}$}
\tadOperacion{cu\'antosLeFaltan}{jugador/j,partida/p}{nat}{$j \leq \text{\#jugadores($p$)}$}
...
\end{flushright}
\tadOperacion{contarNoDominadosHasta}{jugador/j,nat/n,partida/p}{nat}{$j \leq \text{\#jugadores($p$)}$}
\tadOperacion{est\'aDominado?}{casillero/c,partida/p}{bool}{$c \leq \text{\#casilleros(tablero($p$))}$}
\tadOperacion{est\'aDisputado?}{casillero/c,partida/p}{bool}{$c \leq \text{\#casilleros(tablero($p$))}$}
\tadOperacion{jugadoresEnCasillero}{casillero/c,partida/p}{conj(jugador)}{$c \leq \text{\#casilleros(tablero($p$))}$}
\tadOperacion{dominadoPor}{jugador/j,casillero/c,partida/p}{bool}{$j \leq \text{\#jugadores($p$)} \land c \leq \text{\#casilleros(tablero($p$))}$}
...
\end{flushright}
\tadAxioma{fichasEnCasillero($c,$ mover($j,m,n,p$))}{\IF est\'aDominado?($c,p$) THEN {\IF $j \in \text{fichasEnCasillero(c,p)} \land m \in \text{todosLosMovs($c$,tablero($p$))}$ THEN (fichasEnCasillero($c,p$) - agNVeces($j,n,\emptyset$)) $\cup$ \\ fichasVecinasDeJ($c,j,m,p$) ELSE fichasEnCasillero($c,p$) $\cup$ fichasVecinasDeJ($c,j,m,p$) FI} ELSE {\IF est\'aDisputado?(fichasEnCasillero($c,p$)) THEN sinUno(fichasEnCasillero($c,p$)) $\cup$ fichasVecinasDeJ($c,j,m,p$) ELSE fichasVecinasDeJ($c,j,m,p$) FI} FI}
...
\tadAxioma{jugadoresActivos($p$)}{dameActivos($p,$\#jugadores($p$))}

\tadAxioma{jugadoresEliminados($p$)}{dameEliminados($p,$\#jugadores($p$))}
...
\subsection{TAD \tadNombre{SecuExt} extiende \tadNombre{Secuencia}}

\begin{tad}{\tadNombre{SecuExt}}
...
\subsection{TAD \tadNombre{MulticonjExt} extiende \tadNombre{Multiconjunto}}

\begin{tad}{\tadNombre{MulticonjExt}}
