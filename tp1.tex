...
\tadIgualdadObservacional{t}{t'}{tablero}{$(\text{\#}casilleros(t) \igobs \text{\#}casilleros(t'))\ \yluego$\\ $(\forall(c,c':nat)) c,c' \leq \text{\#}casilleros(t) \impluego$\\$ (cont(c,t) \igobs cont(c,t') \land $\\$ movsDeA(c,c',t) \igobs movsDeA(c,c',t') ))$}
...
\tadOperacion{movsDeA}{casillero/c,casillero/c',tab/t}{conj(mov)}{$c, c' \leq \text{\#}casilleros(t)$}
...
\tadAxioma{movsDeA($c,c',$crearTablero($k,k',m,m'$))}{\IF $c=1 \land c'=2$ THEN $\{m\}$ ELSE {\IF $c=2 \land c'=1$ THEN $\{m'\}$ ELSE $\emptyset$ FI } FI}
\tadAxioma{movsDeA($c,c',\text{agregarCasillero($\tilde{c},k,m,m',t$)}$)}{\IF $c = \text{suc(\#casilleros($t$))}$ THEN {\IF $c' = \tilde{c}$ THEN $\{m\}$ ELSE $\emptyset$ FI} ELSE {\IF $c = \tilde{c} \land c' = \text{suc(\#casilleros($t$))} $ THEN $\{m'\}$ ELSE $\text{movsDeA($c,c',t$)}$ FI} FI}
\tadAxioma{movsDeA($c,c',\text{conectar($\tilde{c},\tilde{c}',m,m',t$)}$)}{\IF $c = \tilde{c} \land c' = \tilde{c}' $ THEN $\{m\} \cup \text{movsDeA($c,c',t$)}$ ELSE {\IF $c = \tilde{c}' \land c' = \tilde{c} $ THEN $\{m'\} \cup \text{movsDeA($c,c',t$)}$ ELSE $\text{movsDeA($c,c',t$)}$ FI} FI}
\tadAxioma{movsDeA($c,c',\text{agregarFlecha($\tilde{c},\tilde{c}',m,t$)}$)}{\IF $c = \tilde{c} \land c'=\tilde{c}'$ THEN $\{m\} \cup \text{movsDeA($c,c',t$)}$ ELSE $\text{movsDeA($c,c',t$)}$ FI}
